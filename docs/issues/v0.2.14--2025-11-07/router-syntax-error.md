...
**Zmiany**:
- Poprawiono `fastapi_registry/example_project/app/api/router.py` - zmieniono `except (ImportError, Exception) as e:` na `except ImportError:`
- Poprawiono `fastapi_registry/core/file_utils.py` - funkcja `add_router_to_api_router` teraz:
  - Dodaje importy modułów na początku pliku (nie w try-except)
  - Używa `try-except ImportError` tylko dla opcjonalnych modułów (np. `two_factor`)
  - Wszystkie użycia zmiennych z try-except są wewnątrz tego bloku
