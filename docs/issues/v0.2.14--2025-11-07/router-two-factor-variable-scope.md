...
- Poprawiono `fastapi_registry/core/file_utils.py` - funkcja `add_router_to_api_router`:
  - Dla opcjonalnych modułów (np. `two_factor`) dodaje import i router w jednym bloku `try-except`
  - Wszystkie użycia `two_factor_router` są wewnątrz bloku `try-except`
  - Używa `ImportError` zamiast `Exception` dla lepszej precyzji
  - Sprawdza czy try-except już istnieje i dodaje router do istniejącego bloku
